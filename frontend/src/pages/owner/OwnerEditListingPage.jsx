...
const CITIES = ['Охрид','Скопје','Струга','Битола','Прилеп','Тетово','Гостивар','Крива Паланка','Свети Николе','Крушево','Маврово','Попова Шапка','Дојран','Гевгелија','Куманово','Кавадарци','Струмица','Штип','Велес','Кичево','Кочани','Дебар','Радовиш','Неготино','Делчево','Виница','Ресен','Берово','Кратово','Пробиштип','Богданци','Македонска Каменица','Валандово','Македонски Брод','Демир Капија','Пехчево','Демир Хисар'];
...
const NO_GUESTS = ['office', 'shop', 'other'];
...
export default function OwnerEditListingPage() {
  ...
  const [form, setForm] = useState({
    ...
    listingPurpose: 'izdava', priceType: 'total',
  });
  ...
  useEffect(() => {
    if (existing) {
      ...
      setForm({
        ...
        listingPurpose: existing.listing_purpose || 'izdava',
        priceType: existing.price_type || 'total',
      });
    }
  }, [existing]);
  ...
  const isSelling = form.listingPurpose === 'prodava';
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-900 mb-4">Тип на оглас</h2>
          ...
          {/* Purpose */}
          <div className="mt-4">
            <p className="text-sm font-semibold text-gray-700 mb-3">Цел на огласот</p>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => set('listingPurpose', 'izdava')}
                className={`p-3 rounded-xl border text-sm font-medium transition-all ${form.listingPurpose === 'izdava' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 hover:border-gray-400'}`}>
                🔑 Се издава
              </button>
              <button type="button" onClick={() => set('listingPurpose', 'prodava')}
                className={`p-3 rounded-xl border text-sm font-medium transition-all ${form.listingPurpose === 'prodava' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 hover:border-gray-400'}`}>
                🏷️ Се продава
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
          ...
          {!NO_GUESTS.includes(form.propertyType) && (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
          ...
          {isSelling ? (
            <>
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => set('priceType', 'total')}
                  className={`p-3 rounded-xl border text-sm font-medium transition-all ${form.priceType === 'total' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 hover:border-gray-400'}`}>
                  💰 Вкупна цена
                </button>
                <button type="button" onClick={() => set('priceType', 'per_m2')}
                  className={`p-3 rounded-xl border text-sm font-medium transition-all ${form.priceType === 'per_m2' ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 hover:border-gray-400'}`}>
                  📐 Цена по м²
                </button>
              </div>
              <div className="relative">
                <input type="number" required min="0" value={form.pricePerNight} onChange={(e) => set('pricePerNight', e.target.value)}
                  placeholder={form.priceType === 'per_m2' ? 'Цена по м²' : 'Вкупна цена'} className="input pr-20" />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">
                  {form.priceType === 'per_m2' ? 'МКД/м²' : 'МКД'}
                </span>
              </div>
            </>
          ) : (
        ...
        {/* Rules */}
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-900 mb-3">Опис / Правила (опционално)</h2>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Сакам мојот оглас да биде бесплатно промовиран на социјалните мрежи.</p>
            </div>
          </label>
        </div>
