...
export default function OwnerEditListingPage() {
  ...
  useEffect(() => {
    if (existing) {
      const amenities = Array.isArray(existing.amenities)
        ? existing.amenities
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800">
        ⚠️ По уредувањето, огласот ќе чека повторно одобрување од администраторот.
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          <textarea value={form.rules} onChange={(e) => set('rules', e.target.value)} placeholder="пр. Забранено пушење. Тивко по 22:00..." rows={3} className="input resize-none" />
        </div>
