        ...
        {stepLabels.map((s, i) => (
          <div key={s} className="flex items-center gap-2 flex-1">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-colors ${
              i < step ? 'bg-brand-500 text-white' : i === step ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-500'
            }`}>
              {i < step ? '✓' : i + 1}
            </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-6">
          ...
          <>
                ...
                {PROPERTY_TYPES.map(({ value, label }) => (
                  <button key={value} type="button" onClick={() => setField('propertyType', value)}
                    className={`p-3 rounded-xl border text-sm font-medium transition-all ${
                      form.propertyType === value ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200 hover:border-gray-400'
                    }`}>
                    {label}
                  </button>
                ))}
            ...
            <div>
              <input type="text" value={form.title}
                ...
                placeholder="Наслов на огласот" className="input" maxLength={100} />
            </div>
            <div>
              <textarea value={form.description}
                onChange={(e) => setField('description', e.target.value)}
                placeholder="Опис на просторот..." rows={4} className="input resize-none" />
            </div>

            <div className="grid grid-cols-3 gap-3">
              <div>
                <p className="text-xs text-gray-500 mb-1">Гости (макс)</p>
                <input type="number" min="1" max="50" value={form.maxGuests}
                  onChange={(e) => setField('maxGuests', e.target.value)} className="input" />
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Спални</p>
                <input type="number" min="0" max="20" value={form.bedrooms}
                  onChange={(e) => setField('bedrooms', e.target.value)} className="input" />
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Бањи</p>
                <input type="number" min="0" max="10" value={form.bathrooms}
                  onChange={(e) => setField('bathrooms', e.target.value)} className="input" />
              </div>
            </div>
          ...
          <>
            ...
            <div>
              <input type="text" value={form.address}
                ...
                placeholder="Адреса" className="input" />
            ...
            <div className="relative">
              <input type="number" value={form.pricePerNight}
                ...
                placeholder="Цена по ноќ" min="0" className="input pr-16" />
        ...
        {step === 3 && !isAuthenticated && (
          <>
            ...
            <div className="flex rounded-xl border border-gray-200 p-1">
              {[{ value: 'register', label: 'Нова сметка' }, { value: 'login', label: 'Веќе имам сметка' }].map(({ value, label }) => (
                <button key={value} type="button"
                  onClick={() => { setAccountMode(value); setAccountErrors({}); }}
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <input type="text" value={accountForm.firstName}
                      ...
                      placeholder="Име" className={`input ${accountErrors.firstName ? 'input-error' : ''}`} />
                  ...
                  <div>
                    <input type="text" value={accountForm.lastName}
                      ...
                      placeholder="Презиме" className={`input ${accountErrors.lastName ? 'input-error' : ''}`} />
                    <FieldError error={accountErrors.lastName} />
                  </div>
                </div>
                <div>
                  <input type="text" value={accountForm.email}
                    onChange={(e) => setAccountForm((p) => ({ ...p, email: e.target.value }))}
                    placeholder="Е-маил" className={`input ${accountErrors.email ? 'input-error' : ''}`} />
                ...
                <div>
                  <input type="tel" value={accountForm.phone}
                    ...
                    placeholder="Телефон (пр. 071 234 567)" className={`input ${accountErrors.phone ? 'input-error' : ''}`} />
                ...
                <div>
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={accountForm.password}
                      ...
                      placeholder="Лозинка (мин. 8 знаци)" className={`input pr-11 ${accountErrors.password ? 'input-error' : ''}`} />
                ...
                <div>
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={accountForm.confirmPassword}
                      onChange={(e) => setAccountForm((p) => ({ ...p, confirmPassword: e.target.value }))}
                      placeholder="Потврди лозинка" className={`input pr-11 ${accountErrors.confirmPassword ? 'input-error' : ''}`} />
                    <button type="button" onClick={() => setShowPass((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 p-1">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={eyePath} /></svg>
                    </button>
                  </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={accountForm.password}
                      onChange={(e) => setAccountForm((p) => ({ ...p, password: e.target.value }))}
                      placeholder="Лозинка" className={`input pr-11 ${accountErrors.password ? 'input-error' : ''}`} />
                    <button type="button" onClick={() => setShowPass((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700 p-1">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={eyePath} /></svg>
                    </button>
                  </div>
            ...
            {accountErrors.general && (
              <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-4 py-3">{accountErrors.general}</div>
            )}
        ...
        <div className="flex justify-between pt-4 border-t border-gray-100">
          <button type="button" onClick={() => setStep((p) => p - 1)} disabled={step === 0}
            className="btn-outline rounded-xl px-6 py-2.5 text-sm font-semibold disabled:opacity-0">
            ← Назад
          </button>
          {step < (isAuthenticated ? 2 : 3) ? (
            <button type="button" onClick={() => setStep((p) => p + 1)} disabled={!canProceed()}
              className="btn-primary rounded-xl px-8 py-2.5 text-sm font-semibold disabled:opacity-50">
              Следно →
            </button>
          ) : (
            <button type="button" onClick={handleSubmit} disabled={loading}
              className="btn-primary rounded-xl px-8 py-2.5 text-sm font-semibold disabled:opacity-50 flex items-center gap-2">
